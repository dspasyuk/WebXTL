...
-   **Validation against COD**: `npm test` downloads **205 Crystallography Open Database** structures (covering 179 space-group numbers across all crystal systems) and checks that the determined space group matches the published one (`tests/xrdspace-cod.js`, cached in `HKLs/cod/`). Currently **~94 %** are determined exactly or appear among the zero-violation candidates; the rest are genuine pseudo-symmetry / sparse-data ambiguities (R32 vs R3m, P63 vs P63/m, pseudo-tetragonal orthorhombic cells, etc.).
