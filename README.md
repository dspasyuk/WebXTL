...
-   **HKL Parsing**: Reads **XDS_ASCII.HKL** (headers, unit cell, wavelength), standard **SHELX** five-column HKL files, and **COD `.hkl`** files (CIF-style `_refln_` loops, F² or F).
-   **Validation against COD**: `npm test` downloads **2000 Crystallography Open Database** structures (182 space-group numbers across all crystal systems, cached in `HKLs/cod/`) and checks that the determined space group matches the published one (`tests/xrdspace-cod.js`). It writes `tests/xrdspace-report.json` and a summary chart `tests/xrdspace-report.svg`. On 2000 structures: **~94 % correct** (exact match or among the zero-violation candidates); the rest are genuine pseudo-symmetry / sparse-data / point-group ambiguities (R32 vs R3m, P63 vs P63/m, etc.).
...
-   **Transform Model to Space Group**: when a structure is loaded, the xrdspace results dialog offers *Transform Model to Space Group*. The model's asymmetric unit is expanded under its current symmetry (from `LATT`/`SYMM`) to the full cell content, then reduced to an asymmetric unit under the chosen group — **raising** symmetry removes redundant (symmetry-related) molecules, **lowering** it adds the symmetry partners. Coordinates are fractional (same cell); scattering-factor types, occupancies and ADP columns are preserved, atoms renumbered, and the rewritten `.res` is loaded into the editor for refinement (`POST /xrdspace/transform-model`).
